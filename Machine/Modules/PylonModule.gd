...
func getPylonIdxes():
	...
	for  x in range(pylonRange*2/tilemap.cell_size.x):
		for  y in range(pylonRange*2/tilemap.cell_size.y):
			var posit = Vector2(x,y)+pylonIdx - (floor(pylonRange/tilemap.cell_size.x)*Vector2(1,1))
...
func _draw():
	for idx in pylonIdxes:
		if level.isObstacle(idx) == false && level.getMachineFromIdx(idx) == null: 
			draw_rect(Rect2(-global_position+level.getPosFromCellIdx(idx)+Vector2(tilemap.cell_size*0.25),tilemap.cell_size*0.5),Color('#fcbe03'),false,0.4,false)
		elif level.isObstacle(idx) == false && level.getMachineFromIdx(idx) != null: 
			draw_rect(Rect2(-global_position+level.getPosFromCellIdx(idx),tilemap.cell_size),Color(0,1,1,0.8),false,1.2,false)
