...
func _leftClickWhenNormalState(): 
	...
	if hoveredObject == HOVERED_MODULE:
		var module = level.getModuleFromGlobalIdx(currentMouseIdx)
...
func _attachSelectedModule(idx : Vector2):
	if moduleToAttach != null and selectedModule != null and is_instance_valid(selectedModule):
